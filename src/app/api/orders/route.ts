...
function revalidateOrderDependencies() {
  ...
  try {
    if (Array.isArray(body.items) && body.items.length > 0) {
      if (body.items.length > 100) {
        return NextResponse.json({ error: "В заказе может быть не более 100 позиций" }, { status: 400 });
      }
    ...
    const order = await createOrderQuick(customerId, productId, qty, payment, auth.user.name, auth.user.id);
    revalidateOrderDependencies();
    return NextResponse.json({ order });
  } catch (error) {
    if (error instanceof InventoryError) return NextResponse.json({ error: error.message }, { status: error.status });
    return NextResponse.json({ error: error instanceof Error ? error.message : "Не удалось создать заказ" }, { status: 400 });
  }
}
